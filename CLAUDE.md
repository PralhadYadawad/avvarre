# avvarre — Claude Code Instructions

## Project
avvarre is an MCP server + VS Code agent plugin enforcing Google Style Guides across 21 languages/formats. TypeScript, Node.js, ES modules. Build: `npm run build`. Run: `npm start`.
...
1. **Task sync** — Update `.avvarre/tasks.md` using compressed format: `[x] Summary (steps: a→b→c→d)` for done, `[/] Summary (done: a→b | next: c→d)` for partial. Never expand subtasks in the .md — the step chain IS the expansion key.
2. **Quality nudge** — If source code was written or changed, end with: "Run /avvarre on changed files?"
