#!/usr/bin/env node
// Codex SessionStart hook — auto-detects tech stack and suggests community skills.
...
readInput((input) => {
    ...
    if (!fs.existsSync(avvarreDir)) process.exit(0);

    const detected = {};
    function add(fw, source) { if (!detected[fw]) detected[fw] = source; }
    ...
    if (detectedFrameworks.length === 0) process.exit(0);
    ...
    if (newSuggestions.length === 0) process.exit(0);
